...
export function DailyProgrammeAttachmentDialog({
  open,
  onOpenChange,
  programmeId,
  programmeTitle,
  ...
  const handleDelete = async (attachmentId: string) => {
    ...
    try {
      const response = await fetch(
        `/api/daily-programme/${programmeId}/attachments?attachmentId=${attachmentId}`,
        {
          method: 'DELETE',
        },
      );
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          {attachments.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p>No reference documents attached yet</p>
            </div>
          ) : (
