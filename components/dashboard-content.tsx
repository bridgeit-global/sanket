...
function UpcomingBirthdaysList({
  ...
  birthdays: UpcomingCadreBirthday[];
}) {
  const { t } = useTranslations();
  const [expanded, setExpanded] = useState(false);
  const hasMore = birthdays.length > BIRTHDAY_PREVIEW_LIMIT;
  const visibleBirthdays =
    expanded || !hasMore
      ? birthdays
      : birthdays.slice(0, BIRTHDAY_PREVIEW_LIMIT);
  const hiddenCount = birthdays.length - BIRTHDAY_PREVIEW_LIMIT;
  ...
  return (
    <>
      ...
      {hasMore ? (
        ...
        >
          {expanded
            ? t('dashboard.showFewerBirthdays')
            : t('dashboard.viewMoreBirthdays', { count: hiddenCount })}
        </button>
      ) : null}
    </>
  );
