...
export function AadhaarQrScannerDialog({
  ...
  const handlePayload = useCallback(
    async (payload: string) => {
      try {
        ...
        toast({
          type: 'error',
          description: error instanceof Error ? error.message : 'Could not read Aadhaar QR code',
        });
      }
    },
    [onDataDetected, onOpenChange],
  );
  ...
  useEffect(() => {
    ...
    const start = async () => {
      ...
      try {
        ...
        console.error('Aadhaar QR scanner error:', error);
  ...
  const handleImageUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      console.error('Aadhaar image decode error:', error);
      toast({
        type: 'error',
      ...
      <DialogContent className="max-w-xl">
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:justify-between">
          ...
          >
            ...
            {isProcessingImage ? 'Processing image…' : 'Upload Aadhaar photo'}
          </Button>
          <input
            ref={fileInputRef}
            type="file"
